import os                                              # builds the database file path and creates the instance folder
from datetime import datetime, timedelta               # datetime for timestamps, timedelta for the 2-hour study room cutoff
from flask import Flask, request                       # Flask = app factory, request = current HTTP request (used in inject_global_data)
from flask_login import current_user                   # the currently logged-in user (used in inject_global_data)
from config import Config                              # Config class with SECRET_KEY, MAIL_*, etc.
from app.extensions import db, login_manager, mail     # pre-created extension objects, attached to the app via init_app()
...
def create_app(config_class=Config):
    ...
    # for: study rooms, chat rooms, notifications which are route independent.
